	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ex 6D -- 3 Formas Juntas -- Ricardo Moreira Gomes\tFPS %.2lf", fps);
		...
		glUniform4f(colorLoc, 1.0, 0.0f, 0.0f, 1.0f); // enviando cor para variável uniform inputColor

		// Chamada de desenho - drawcall
		// Poligono Preenchido - GL_TRIANGLES
		glDrawArrays(GL_TRIANGLES, 0, 6);

		glUniform4f(colorLoc, 1.0, 1.0f, 0.0f, 1.0f);
		glDrawArrays(GL_LINE_LOOP, 0, 3);
		glDrawArrays(GL_LINE_LOOP, 3, 3);
		
		glUniform4f(colorLoc, 1.0, 0.0f, 1.0f, 1.0f);
