...
const int NUM_SEGMENTS = 50; //numero de segmentos do circulo
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ex 7C -- Pac-Man -- Ricardo Moreira Gomes\tFPS %.2lf", fps);
		...
		//Desenho circulo com boca faltando
...
int setupGeometry()
{
	...
	const float mouthAngle = 60.0f * (M_PI / 180.0f);
    ...
    // === Parte 1: Lado DIREITO da boca (começa aqui) ===
    vertices.push_back(radius * cos(mouthAngle / 2.0f)); // Ponto A
    vertices.push_back(radius * sin(mouthAngle / 2.0f));
    vertices.push_back(0.0f);

    // === Parte 2: Arco do círculo (faltando a boca) ===
    for (int i = 1; i < NUM_SEGMENTS; ++i) {
        float theta = (mouthAngle / 2.0f) + (2 * M_PI - mouthAngle) * float(i) / float(NUM_SEGMENTS);
        vertices.push_back(radius * cos(theta)); // x
        vertices.push_back(radius * sin(theta)); // y
        vertices.push_back(0.0f);               // z
    }

    // === Parte 3: Lado ESQUERDO da boca (termina aqui) ===
    vertices.push_back(radius * cos(-mouthAngle / 2.0f)); // Ponto B
    vertices.push_back(radius * sin(-mouthAngle / 2.0f));
    vertices.push_back(0.0f);
