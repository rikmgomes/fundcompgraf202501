...
const int NUM_SEGMENTS = 5; //numero de segmentos do circulo
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ex 7B -- Pentágono -- Ricardo Moreira Gomes\tFPS %.2lf", fps);
		...
		glDrawArrays(GL_LINE_LOOP, 0, NUM_SEGMENTS + 1); //+1 porque o loop vai de 0 a NUM_SEGMENTS
...
int setupGeometry()
{
	...
	// Evitar elipse (circulo achatado por conta da resolução da janela)
	float aspectRatio = (float)WIDTH / (float)HEIGHT;
    
    // Geração dos vértices usando a equação paramétrica do círculo
