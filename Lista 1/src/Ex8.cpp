...
int main()
{
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Ex 8 -- Espiral -- Ricardo Moreira Gomes", nullptr, nullptr);
	if (!window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ex 8 -- Espiral -- Ricardo Moreira Gomes\tFPS %.2lf", fps);
		...
		//Desenho espiral
		glUniform4f(colorLoc, 1.0f, 0.0f, 0.0f, 1.0f); //espiral vermelha
		glDrawArrays(GL_LINE_STRIP, 0, NUM_SEGMENTS +1); //contorno contínuo
...
int setupGeometry()
{
    const float a = 0.1f; //raio inicial
	const float b = 0.05f; //fator de crescimento
	const float maxTheta = 3.0f * 2.0f * M_PI; //Nvoltas *2.0f *mpi
    vector<GLfloat> vertices;
	float aspectRatio = (float)WIDTH / (float)HEIGHT;
    
    for (int i = 0; i <= NUM_SEGMENTS; ++i) {
        float theta = maxTheta * float(i) / float(NUM_SEGMENTS);
        float r = a + b * theta; // Equação da espiral

        float x = r * cos(theta) / aspectRatio;
        float y = r * sin(theta);
