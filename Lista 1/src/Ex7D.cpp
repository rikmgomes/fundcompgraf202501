...
const int NUM_SEGMENTS = 20; //numero de segmentos do circulo
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ex 7D -- Fatia de Pizza -- Ricardo Moreira Gomes\tFPS %.2lf", fps);
		...
		glDrawArrays(GL_TRIANGLE_FAN, 0, NUM_SEGMENTS +2); //preenchimento
...
int setupGeometry()
{
	...
	const float sliceAngle = 60.0f;
    vector<GLfloat> vertices;
	float aspectRatio = (float)WIDTH / (float)HEIGHT;
    
    // === Ponto central (ponta da fatia) ===
    vertices.push_back(0.0f); // x
    vertices.push_back(0.0f); // y
    vertices.push_back(0.0f); // z

    // === Arco da borda (parte redonda) ===
    for (int i = 0; i <= NUM_SEGMENTS; ++i) {
        float theta = (sliceAngle / 2.0f) - (sliceAngle * float(i) / float(NUM_SEGMENTS));
        theta *= (M_PI / 180.0f); // Converte para radianos

        float x = radius * cos(theta) / aspectRatio;
