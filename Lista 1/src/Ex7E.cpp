...
const int NUM_SEGMENTS = 100; //numero de segmentos do circulo
	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ex 7E -- Desafio (Estrela) -- Ricardo Moreira Gomes\tFPS %.2lf", fps);
		...
		glDrawArrays(GL_LINE_LOOP, 0, 11); // 11 vértices (5 pontas + 5 reentrantes + fechamento)
...
int setupGeometry() {
    const float outerRadius = 0.5f; // Raio das pontas
    const float innerRadius = 0.2f; // Raio dos "reentrantes"
    const int numPoints = 5;        // 5 pontas

    vector<GLfloat> vertices;

    for (int i = 0; i <= numPoints * 2; ++i) {
        float theta = (i * 2.0f * M_PI) / (numPoints * 2); // Ângulo atual
        float radius = (i % 2 == 0) ? outerRadius : innerRadius; // Alterna entre raios

        float x = radius * cos(theta - M_PI / 2.0f); // -M_PI/2 para rotacionar a estrela
        float y = radius * sin(theta - M_PI / 2.0f);
